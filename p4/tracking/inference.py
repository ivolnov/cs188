...
class ExactInference(InferenceModule):
  ...
  def observe(self, observation, gameState):
    ...
    if noisyDistance is None:
      ...
      for position in self.beliefs:
        self.beliefs[position] = 1 if position == jail else 0
      return

    for position in self.legalPositions:
      trueDistance = util.manhattanDistance(position, pacmanPosition)
      conditional = emissionModel[trueDistance]
      self.beliefs[position] = self.beliefs[position] * conditional
  ...
  def elapseTime(self, gameState):
    ...
    for position, marginal in self.beliefs.items():
      if marginal != 0:
        self.setGhostPosition(gameState, position)
        successors = self.getPositionDistribution(gameState)
        for successorPosition, conditional in successors.items():
          distribution[successorPosition] += marginal * conditional
...
class ParticleFilter(InferenceModule):
  ...
  def observe(self, observation, gameState):
    ...
    if noisyDistance is None:
      ...
      if all(probability == 0 for probability in distribution):
        self.initializeUniformly(gameState)
      else:
        self.particles = [util.sample(distribution) for i in xrange(self.numParticles)]
  ...
  def elapseTime(self, gameState):
    ...
    for i in xrange(self.numParticles):
      state = self.setGhostPosition(gameState, self.particles[i])
...
class JointParticleFilter:
  ...
  def initializeParticles(self):
    ...
    for i in xrange(self.numParticles):
      ...
      for j in xrange(self.numGhosts):
        positions.append(random.choice(self.legalPositions))
  ...
  def elapseTime(self, gameState):
    ...
    for oldParticle in self.particles:
      ...
      for i in xrange(self.numGhosts):
        distribution = getPositionDistributionForGhost(state, i, self.ghostAgents[i])
  ...
  def observeState(self, gameState):
    ...
    for particle in self.particles:
      ...
      for ghost, noise in enumerate(noisyDistances):
        if noise is None:
          sample.append(self.getJailPosition(ghost))
        else:
          distance = util.manhattanDistance(particle[ghost], pacmanPosition)
          weight *= emissionModels[ghost][distance]
          sample.append(particle[ghost])

      weights[tuple(sample)] += weight
      ...
      for i in xrange(self.numParticles):
        ...
        for ghost, noise in enumerate(noisyDistances):
          if noise is None:
            ...
            sample.append(random.choice(self.legalPositions))

        self.particles.append(tuple(sample))
    else:
